...
| `solve_pde()` — `selector_type="tag"` conditions (32/65 presets tag-only, 8/65 mixed) were silently dropped when `ctx["tag_masks"]`/explicit `x_bc`+`mask_<name>` weren't supplied, training with **zero boundary conditions enforced** and no error — a physically invalid "success" a trust-score consumer could rate as trustworthy. Verified with real geometry first (`STLDomainBatchBuilder` + a real trimesh STL, `examples/pde_environment/04_heat3d_stl_box.py`) that the tag_masks mechanism itself is correct, not the bug; then made `solve_pde()` raise `TagConditionsUnresolved` naming the exact missing condition(s)/tag(s) instead of training silently | ✅ done, full before/after test-suite numbers and preset classification table in `AUDIT_REPORT.md` (`tests/test_cartesian_breadth.py` 49/21/0 → 14/56/0; `tests/test_full_library_matrix.py` 75/77/7 → 42/117/0) | `pinneapple_physics/__init__.py`, `tests/test_full_library_matrix.py`, `tests/test_cartesian_breadth.py` |
