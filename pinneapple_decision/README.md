...
| `hybrid_surrogate_physics` | `has_physics_postprocessor=True` (target derived from the flow by an established physics model, e.g. erosion) | `pinneapple_neural.workflows.hybrid_surrogate_physics.HybridSurrogatePhysics` | surrogate of the intermediate quantities + the user's explicit physics post-model (`PhysicsPostModel`, with name and source); Monte Carlo propagation of the surrogate's uncertainty; `with_post_model` swaps the physics without retraining |
