...
def test_audit_breadth_preset_trains_a_few_steps(name):
    ...
    extra_kwargs = {}
    if name in TAG_GEOMETRY_FIXTURES:
        try:
            batch = build_tag_batch(name, spec, n_col=64, n_bc_per_face=32, seed=0)
            extra_kwargs = solve_pde_kwargs_from_batch(batch)
        except Exception as e:
            pytest.fail(f"build_tag_batch('{name}') raised: {e}")

    try:
        result = pp.solve_pde(spec, model, epochs=3, n_collocation=64, n_condition=32, **extra_kwargs)
    except Exception as e:
        if _is_missing_optional_dep(e):
            pytest.skip(f"preset '{name}' needs an optional dependency not installed: {e}")
